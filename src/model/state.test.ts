...
test("state calculates committee weights correctly: New weight calc ", (t) => {
  const s = new State();
  testCommittee.Members.map(
    (m: { EthAddress: string; Weight: number; EffectiveStake: number }) => {
  ...
  for (const node of s.getSnapshot().CurrentCommittee) {
    t.is(
      s.getSnapshot().CurrentEffectiveStake[node.EthAddress],
      node.EffectiveStake
    );
  }
  t.is(
    _.filter(
      ...
      (m) => m.EthAddress == "f7ae622c77d0580f02bcb2f92380d61e3f6e466c"
    )[0].Weight,
    _.filter(
      testCommittee.Members,
      (m) => m.EthAddress == "f7ae622c77d0580f02bcb2f92380d61e3f6e466c"
    )[0].Weight
  );
  ...
  const totalWeight = _.sum(
    _.map(s.getSnapshot().CurrentCommittee, (node) => node.Weight ?? 0)
  );
  const totalStake = _.sum(
    _.map(s.getSnapshot().CurrentCommittee, (node) => node.EffectiveStake ?? 0)
  );
...
test("state applies elections status updates and sets candidates accordingly", (t) => {
  ...
  t.assert(
    s.getSnapshot().CurrentElectionsStatus["b"].TimeToStale == 7 * 24 * 60 * 60
  );
...
test("state applies image version changes", (t) => {
  ...
  t.is(
    s.getSnapshot().CurrentImageVersions["main"]["management-service"],
    "v1.0.1"
  );
  ...
  t.deepEqual(
    s.getSnapshot().CurrentImageVersionsUpdater["main"]["management-service"],
    {
      LastPollTime: 2000,
      PendingVersion: "",
      PendingVersionTime: 0,
...
test("state applies meta registration data", (t) => {
  ...
  t.is(
    s.getSnapshot().CurrentRegistrationData["a"].Metadata[
...
test("state update events stats", (t) => {
  ...
  t.is(
    s.getSnapshot().EventsStats.TotalEventsProcessed,
  ...
  t.is(
    s.getSnapshot().EventsStats.EventCount["ProtocolVersionChanged"].Count,
    1
  );
});

function ContractAddressUpdated(
  s: State,
  time: number,
...
function GuardianCommitteeWeight(
  s: State,
  addr: string,
  weight: string,
  inCommittee: boolean,
...
function GuardianMetadataChanged(
  s: State,
  time: number,
  guardian: string,
  key: string,
...
function ProtocolVersionChanged(
  s: State,
  time: number,
  nextVersion: number,
...
function GuardianCertificationUpdate(
  s: State,
  time: number,
  guardian: string,
  isCertified: boolean
