...
const committee_example_test_json_1 = require("./committee_example_test.json");
const lodash_1 = __importDefault(require("lodash"));
(0, ava_1.default)("state applies time ref and ref block", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state applies commitee, standby, IPs and topology", (t) => {
    const s = new state_1.State();
    ...
    GuardianDataUpdated(s, helpers_1.day + 3000, "0xX", "0xx3", "0x09090909");
    GuardianDataUpdated(s, helpers_1.day + 3000, "0xZ", "0xz3", "0x0a0a0a0a");
    GuardianDataUpdated(s, helpers_1.day + 3000, "0xZ", "0xz3", "0x0b0b0b0b");
    GuardianDataUpdated(s, helpers_1.day + 3000, "0xP", "0xp3", "0x0c0c0c0c");
    GuardianDataUpdated(s, helpers_1.day + 3000, "0xO", "0xo3", "0x08080808");
    GuardianStatusUpdated(s, helpers_1.day + 3000, "0xX", true, false);
    GuardianStatusUpdated(s, helpers_1.day + 3000, "0xP", true, false);
    // change committee to [X, Z]
    CommitteeChange(s, helpers_1.day + 3000, "0xB", false);
    CommitteeChange(s, helpers_1.day + 3000, "0xC", false);
    CommitteeChange(s, helpers_1.day + 3000, "0xX", true);
    // change standbys to [A, B, C, M, N]
    CommitteeChange(s, helpers_1.day + 3000, "0xN", false);
    CommitteeChange(s, helpers_1.day + 3000, "0xP", false);
    StakeChanged(s, helpers_1.day + 3000, "0xO", "20000000000000000000000");
    s.applyNewTimeRef(helpers_1.day + 3000, 300);
    ...
    t.deepEqual(s.getSnapshot().CommitteeSets[0].CommitteeEthAddresses, [
        "a",
        "b",
        "c",
    ]);
    t.is(s.getSnapshot().CommitteeSets[1].RefTime, 2000);
    t.deepEqual(s.getSnapshot().CommitteeSets[1].CommitteeEthAddresses, [
        "b",
        "c",
        "z",
    ]);
    t.is(s.getSnapshot().CommitteeSets[2].RefTime, helpers_1.day + 3000);
    t.deepEqual(s.getSnapshot().CommitteeSets[2].CommitteeEthAddresses, [
        "x",
        "z",
    ]);
    ...
    t.is(s.getSnapshot().CommitteeEvents[2].RefTime, helpers_1.day + 3000);
    ...
    CommitteeChange(s, helpers_1.day + 5000, "0xZ", false);
    CommitteeChange(s, helpers_1.day + 6000, "0xZ", true);
    t.is(s.getSnapshot().CurrentCommittee[1].EnterTime, helpers_1.day + 6000);
    CommitteeChange(s, helpers_1.day + 7000, "0xZ", true);
    t.is(s.getSnapshot().CurrentCommittee[1].EnterTime, helpers_1.day + 6000);
    ...
    GuardianDataUpdated(s, helpers_1.day + 8000, "0xB", "0xb2", "0x02020202", false);
    GuardianDataUpdated(s, helpers_1.day + 8000, "0xM", "0xm1", "0x04040404", false);
    s.applyNewTimeRef(helpers_1.day + 8000, 800);
...
(0, ava_1.default)("state handles guardian unregister", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state calculates committee weights correctly and guardian stake", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state calculates committee weights correctly: New weight calc ", (t) => {
    const s = new state_1.State();
    committee_example_test_json_1.Committee.Members.map((m) => {
        const weight = m.EffectiveStake.toString();
        GuardianCommitteeWeight(s, m.EthAddress, weight.padEnd(weight.length + 18, "0"), true, state_1.NEW_FIX_COMMITTEE_WEIGHTS_BREAKING_CHANGE_TIME - 1);
    });
    t.is(s.getSnapshot().CurrentCommittee.length, committee_example_test_json_1.Committee.Members.length);
    ...
    t.is(lodash_1.default.filter(s.getSnapshot().CurrentCommittee, (m) => m.EthAddress == "f7ae622c77d0580f02bcb2f92380d61e3f6e466c")[0].Weight, lodash_1.default.filter(committee_example_test_json_1.Committee.Members, (m) => m.EthAddress == "f7ae622c77d0580f02bcb2f92380d61e3f6e466c")[0].Weight);
    committee_example_test_json_1.Committee.Members.map((m) => {
        const weight = m.EffectiveStake.toString();
        GuardianCommitteeWeight(s, m.EthAddress, weight.padEnd(weight.length + 18, "0"), true, state_1.NEW_FIX_COMMITTEE_WEIGHTS_BREAKING_CHANGE_TIME);
    });
    t.assert(lodash_1.default.filter(s.getSnapshot().CurrentCommittee, (m) => m.EthAddress == "f7ae622c77d0580f02bcb2f92380d61e3f6e466c")[0].Weight >
        lodash_1.default.filter(committee_example_test_json_1.Committee.Members, (m) => m.EthAddress == "f7ae622c77d0580f02bcb2f92380d61e3f6e466c")[0].Weight);
    const totalWeight = lodash_1.default.sum(lodash_1.default.map(s.getSnapshot().CurrentCommittee, (node) => { var _a; return (_a = node.Weight) !== null && _a !== void 0 ? _a : 0; }));
    const totalStake = lodash_1.default.sum(lodash_1.default.map(s.getSnapshot().CurrentCommittee, (node) => { var _a; return (_a = node.EffectiveStake) !== null && _a !== void 0 ? _a : 0; }));
    const top3Sum = [
        lodash_1.default.sumBy(s.getSnapshot().CurrentCommittee.slice(0, 3), "Weight"),
        lodash_1.default.sumBy(s.getSnapshot().CurrentCommittee.slice(0, 3), "EffectiveStake"),
    ];
    t.assert(top3Sum[0] / totalWeight < 1 / 3);
    t.assert(top3Sum[1] / totalStake < 2 / 3);
});
(0, ava_1.default)("state applies elections status updates and sets candidates accordingly", (t) => {
    const s = new state_1.State();
    GuardianDataUpdated(s, 1000, "0xA", "0xa1", "0x01010101");
    ...
    GuardianStatusUpdated(s, 1 * helpers_1.day, "0xD", true, false);
    GuardianStatusUpdated(s, 1 * helpers_1.day, "0xX", true, false);
    GuardianStatusUpdated(s, 1 * helpers_1.day, "0xY", true, false);
    GuardianStatusUpdated(s, 1 * helpers_1.day, "0xZ", true, false);
    GuardianStatusUpdated(s, 5 * helpers_1.day, "0xA", true, false);
    ...
    s.applyNewTimeRef(10 * helpers_1.day, 10000);
    t.log(JSON.stringify(s.getSnapshot(), null, 2));
    ...
    GuardianStatusUpdated(s, 11 * helpers_1.day, "0xA", false, false);
    GuardianStatusUpdated(s, 11 * helpers_1.day, "0xC", false, false);
    s.applyNewTimeRef(11 * helpers_1.day, 20000);
    t.log(JSON.stringify(s.getSnapshot(), null, 2));
    t.deepEqual(s.getSnapshot().CurrentCandidates, [
        { EthAddress: "d", IsStandby: true, Name: "name" },
        { EthAddress: "x", IsStandby: true, Name: "name" },
...
(0, ava_1.default)("state applies virtual chain subscriptions", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state applies protocol version changes", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state applies image version changes", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state applies meta registration data", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state applies contract address changes", (t) => {
    const s = new state_1.State();
...
(0, ava_1.default)("state update events stats", (t) => {
    const s = new state_1.State();
    const events1 = [
...
function ContractAddressUpdated(s, time, contractName, addr) {
    s.applyNewContractAddressUpdated(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            contractName,
            addr,
            managedContract: false,
        } }));
}
function CommitteeChange(s, time, addr, inCommittee) {
    s.applyNewCommitteeChange(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            addr,
...
function GuardianCommitteeWeight(s, addr, weight, inCommittee, time = 1000) {
    s.applyNewCommitteeChange(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            addr,
            weight,
            certification: false,
...
function StakeChanged(s, time, addr, effectiveStake) {
    s.applyNewStakeChanged(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            addr,
...
function GuardianDataUpdated(s, time, guardian, orbsAddr, ip, isRegistered = true) {
    s.applyNewGuardianDataUpdated(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            ip,
            guardian,
            orbsAddr,
            name: "name",
...
function GuardianMetadataChanged(s, time, guardian, key, value) {
    s.applyNewGuardianMetadataChanged(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            guardian,
            key,
            newValue: value,
            oldValue: "unknown",
...
function SubscriptionChanged(s, time, vcId, expiresAt) {
    s.applyNewSubscriptionChanged(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            name: "name",
            owner: "owner",
            vcId,
            genRefTime: "9999",
...
function ProtocolVersionChanged(s, time, nextVersion, fromTimestamp) {
    s.applyNewProtocolVersionChanged(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            deploymentSubset: "main",
            currentVersion: "xxx",
...
function GuardianStatusUpdated(s, time, guardian, readyToSync, readyForCommittee) {
    s.applyNewGuardianStatusUpdated(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            guardian,
            readyToSync,
            readyForCommittee,
        } }));
}
function GuardianCertificationUpdate(s, time, guardian, isCertified) {
    s.applyNewGuardianCertificationUpdate(time, Object.assign(Object.assign({}, eventBase), { returnValues: {
            guardian,
            isCertified,
        } }));
}
