...
export async function jettonWalletBalance(
  ...
  let res = await client4.runMethod(
    snapshot.Proposals[vote.proposalId].snapshotLogicalTime,
    Address.parse(vote.voter),
    "get_wallet_address"
  );
  ...
  res = await client4.runMethod(
    snapshot.Proposals[vote.proposalId].snapshotLogicalTime,
    Address.parse(voterJettonWalletAddress),
    "get_wallet_data"
  );
