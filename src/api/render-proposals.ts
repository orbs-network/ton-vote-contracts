...
export function renderProposalResult(snapshot: StateSnapshot, proposalId: string) {
...
export function renderActiveProposals(snapshot: StateSnapshot, daoId: string) {
	_.filter(snapshot.Proposals, (proposal) => (daoId in proposal) && proposal.timestamp + proposal.duration < Date.now())
}
