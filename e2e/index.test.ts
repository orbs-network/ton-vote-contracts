...
import {
  expectationVcManagement,
  expectationHistoricVcManagement,
} from "./expectations-vc";
...
test.serial.before(async (t) => {
  ...
  await driver.ethereum.increaseBlocks(
    driver.getAppConfig(false).FinalityBufferBlocks
  );
...
test.serial(
  "[E2E] serves /vchains/1000000/management as expected",
  async (t) => {
    t.log("started");
    driver.testLogger = t.log;
    t.timeout(60 * 1000);
    ...
    while (
      !res ||
      isErrorResponse(res) ||
      res.CurrentRefTime < stateReadyBlockTime
    ) {
      await sleep(1000);
      console.log("fetching node/management again, since last response:", res);
      t.log(
        "fetching vchains/1000000/management again, since last response:",
        res
      );
...
test.serial(
  "[E2E] serves /vchains/1000000/management/time as expected",
  async (t) => {
    t.log("started");
    driver.testLogger = t.log;
    t.timeout(60 * 1000);
    ...
    let res = await driver.fetch(
      "app",
      8080,
      `vchains/1000000/management/${stateReadyBlockTime}`
    );
    while (!res || isErrorResponse(res)) {
      await sleep(1000);
      console.log(
        "fetching node/management/time again, since last response:",
        res
      );
      t.log(
        "fetching vchains/1000000/management/time again, since last response:",
        res
      );
      res = await driver.fetch(
        "app",
        8080,
        `vchains/1000000/management/${stateReadyBlockTime}`
      );
    ...
    res = await driver.fetch(
      "app",
      8080,
      `vchains/1000000/management/${stateReadyBlockTime + year}`
    );
...
function isErrorResponse(
  res: any
): res is { error: string; stack?: string | undefined; status: "error" } {
