...
export class TestEnvironment {
  ...
  getAppConfig(bootstrapMode: boolean) {
    return {
      ...
      DeploymentDescriptorUrl:
        "https://deployment.orbs.network/e2e/management-service-test.json", // TODO: move to local docker
    };
  }
  ...
  launchServices(bootstrapMode: boolean) {
    ...
    test.serial.before(async (t) => {
      ...
      t.timeout(2 * 60 * 1000);
      const ganacheAddress = await getAddressForService(
        this.envName,
        this.pathToDockerCompose,
        "ganache",
        7545
    ...
    test.serial.before((t) => {
      ...
      writeFileSync(
        configFilePath,
        JSON.stringify(this.getAppConfig(bootstrapMode))
      );
    });
    ...
    test.serial.before(async (t) => {
      ...
      const logP = exec(
        `docker-compose -p ${this.envName} -f "${this.pathToDockerCompose}" logs -f app`
      );
  ...
  async fetch(serviceName: string, port: number, path: string) {
    const addr = await getAddressForService(
      this.envName,
      this.pathToDockerCompose,
      serviceName,
      port
