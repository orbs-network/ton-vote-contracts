...
const registry_1 = require("../contracts-ts/registry");
const dao_1 = require("../contracts-ts/dao");
const proxy_1 = require("../contracts-ts/proxy");
const ton_1 = require("ton");
const chai_1 = require("chai");
const buffer_1 = require("buffer");
const ton_access_1 = require("@orbs-network/ton-access");
const elector = ton_1.Address.parse("Ef8zMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzMzM0vF");
...
function deployContract(client, walletContract, privateKey, contract, contractParams = undefined) {
...
describe("e2e test suite", () => {
    ...
    it("Deploy Registry contract", () => __awaiter(void 0, void 0, void 0, function* () {
        registryContract = yield deployContract(client, deployWallet, deployWalletKey.secretKey, registry_1.Registry);
    }));
    it.only("Deploy Proxy contract", () => __awaiter(void 0, void 0, void 0, function* () {
        let proxy_id = 0;
        let next_child_id = 0;
        proxyContract = yield deployContract(client, deployWallet, deployWalletKey.secretKey, proxy_1.Proxy, { owner: deployWallet.address, proxyId: proxy_id, next_child_id: next_child_id, parent_addr: deployWallet.address, content: (0, ton_1.beginCell)().storeUint(0xcaffe, 64).endCell() });
        res = yield client.callGetMethod(proxyContract.address, 'get_owner_addr');
        (0, chai_1.expect)(bytesToAddress(res.stack[0][1].bytes).toFriendly()).to.eq(deployWallet.address.toFriendly());
        res = yield client.callGetMethod(proxyContract.address, 'get_proxy_id');
        (0, chai_1.expect)(Number(res.stack[0][1])).to.eq(proxy_id);
        res = yield client.callGetMethod(proxyContract.address, 'get_next_child_id');
        (0, chai_1.expect)(Number(res.stack[0][1])).to.eq(next_child_id);
    }));
    ...
    it("Read get_next_dao_id from registry", () => __awaiter(void 0, void 0, void 0, function* () {
        daoSettings = (0, ton_1.beginCell)().storeUint(0, 64).endCell();
        daoContract = dao_1.Dao.create(registryContract, 0, deployWallet.address, daoSettings);
        let daoAddr;
        payload = (0, ton_1.beginCell)().storeUint(OP_NEW_DAO, 32).storeRef(daoSettings).endCell();
        res = yield sendMsg(client, deployWallet, deployWalletKey.secretKey, registryContract.address, SEND_MSG_VALUE, payload);
    }));
